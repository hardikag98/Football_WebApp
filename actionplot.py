...
Refactored: 2025 — socceraction/HDF5 dependencies commented out for initial Render deployment.
Uncomment the original plotaction() and imports to restore full Goals tab functionality.
"""
import os
import warnings
import pandas as pd
...
def plotaction(match_id, number=5, w=1, h=1, zoom=False):
    """
    Placeholder while socceraction is disabled.
    Returns a pitch image with a 'coming soon' message.
    """
    fig = plt.figure()
    ...
    MPS.drawactionfield(
        ...
        title="Goals view — coming soon (requires SPADL data)"
    )
    plt.tight_layout()
    buf = io.BytesIO()
...
# ──────────────────────────────────────────────────────────────
# ORIGINAL plotaction (requires socceraction + spadl-statsbomb.h5)
# Uncomment this and the socceraction imports above to restore.
# ──────────────────────────────────────────────────────────────
#
# def plotaction(match_id, number=5, w=1, h=1, zoom=False):
...
#
#     goalindex = actions[
#         (actions.type_name == 'shot') | (actions.type_name == 'shot_penalty')
#     ][(actions.result_name == 'success') | (actions.result_name == 'owngoal')].index
#
#     if len(goalindex) == 0:
#         fig = plt.figure()
#         fig.set_size_inches(6.7 * 1.5, 6.7, forward=False)
